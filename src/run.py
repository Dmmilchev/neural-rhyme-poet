...
if len(sys.argv)>1 and sys.argv[1] == 'prepare':
    testCorpus, trainCorpus, char2id, auth2id =  corpus_prepare.prepare_data(corpusFileName, startChar, endChar, unkChar, padChar)
...
if len(sys.argv)>1 and sys.argv[1] == 'train':
    trainCorpus, char2id = corpus_prepare.load_corpus(trainDataFileName)

    stress_model = accentor.StressTransformer()
    stress_model.load_state_dict(torch.load('/workspace/Neural Poet/models/stress_transformer14.pth'))
    ...
    train.trainModel(trainCorpus, lm, optimizer, epochs, batchSize, stress_model)
    # lm.save(modelFileName + f".final")
